...
export const AttendanceScreen: React.FC = () => {
  ...
  const [steps, setSteps] = useState<WorkflowStep[]>([
    { id: 'permission', label: 'Location Permission', completed: false, loading: false },
    { id: 'location', label: 'Getting Location', completed: false, loading: false },
    { id: 'radius', label: 'Verifying Office Radius', completed: false, loading: false },
  ...
  // Update step status
  const updateStep = (id: WorkflowStep['id'], updates: Partial<WorkflowStep>) => {
    setSteps((prevSteps) =>
      ...
      if (startResult.location) {
        ...
        if (!isValid) {
          ...
          Toast.show({
            ...
            text2: 'You are outside office radius. Please move closer to office.',
          });
          return;
        }
    ...
    try {
      ...
      if (location) {
        const completeResult = isCheckIn
          ? await AttendanceWorkflowService.completeCheckIn(user!.id, location, base64Data)
          : await AttendanceWorkflowService.completeCheckOut(user!.id, location, base64Data);
        ...
        if (completeResult.success) {
          ...
          Toast.show({
            type: 'success',
            text1: 'Success',
            text2: completeResult.message || `${isCheckIn ? 'Check-in' : 'Check-out'} recorded`,
          });
  ...
  const convertImageToBase64 = async (uri: string): Promise<string> => {
    try {
      // For URI from camera, we need to read the file
  ...
  const handleRetry = () => {
    setWorkflowStarted(false);
    setSteps((prev) =>
      prev.map((step) => ({ ...step, completed: false, loading: false, error: undefined }))
    );
    setLocation(null);
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Icon name={isCheckIn ? 'login' : 'logout'} size={24} color="#0066cc" />
